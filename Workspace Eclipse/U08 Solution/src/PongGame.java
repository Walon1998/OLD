...
public class PongGame {
    ...
    public void step() {
        if(ballAddCounter >= 50) {
            ...
            balls.addLast(new Ball(width/2, height/2, speed * Math.cos(angle), speed * Math.sin(angle)));
            ballAddCounter = 0;
        }
        ballAddCounter++;
        
        for(BallNode n = balls.first; n != null; n = n.next) {
            Ball ball = n.ball;
            if(ball.x < 0) {
                p2.score++;
                balls.removeNode(n);
